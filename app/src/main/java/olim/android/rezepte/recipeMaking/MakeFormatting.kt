...
class MakeFormatting {
    companion object {
            ...
            get() {
                ...
                if (number != null) {
                    output += if (number.value.contains("/")) {//if it is a fractional value work that out
                        ...
                        number.value.toFloatOrNull()
                            ?: 0f //convert to float but if its somehow invalid return 0
        ...
        private fun String.replace(replacements: List<String>, replacement: String): String {
            var result = this
            replacements.forEach { oldWord ->
                result = result.replaceFirst(oldWord, replacement)
            } //todo could replace part of a word instead of the unit i think
            return result
        }
        ...
        private fun String?.indexesOf(pat: String, ignoreCase: Boolean = true): List<Int> =
            //returns every index a value is found
            pat.toRegex(ignoreCaseOpt(ignoreCase))
                .findAll(this ?: "")
                .map { it.range.first }
                .toList()

        private fun String?.rangesOf(pat: String, ignoreCase: Boolean = true): List<IntRange> =
            //returns every index a value is found
            pat.toRegex(ignoreCaseOpt(ignoreCase))
                .findAll(this ?: "")
                .map { it.range }
        ...
        private fun String.removeMeasurement(
            ...
            for (location in slashLocation) {
                if (location < bestNumberIndex) {
                    if (this.substring(location + 1, bestNumberIndex)
                            .matches("\\s*".toRegex())
                    ) {//if the gap between is just spaces move the index down to that location
                        bestNumberIndex = location
                    }
                } else if (location > bestEndIndex) {
                    if (this.substring(bestEndIndex + 1, location)
                            .matches("\\s*".toRegex())
                    ) {//the same but for the end index
        ...
        private fun combineNumbersWithThereFractions(string: String): String {
            ...
            val cleanWords =
                removeUnnecessarySlash(slitTextFromNumbers(getWordsWithNumbers(string)))
        ...
        fun getCorrectUnitsAndValuesInIngredients(
            string: String,
            multiplier: Float,
            ...
            return multiplyByOnlyValues(
                output,
                multiplier,
                settings["Units.Fractional Numbers"] == "true",
                if (settings["Units.Round Numbers"] == "true") 0.02f else -1f
            )
        }

        fun getCorrectUnitsAndValues(
            string: String,
            multiplier: Float,
            settings: Map<String, String>
        ): String {
        ...
        fun listUnitsInValue(string: String): List<String> {
            ...
            val orderedUnitIndexes =
                unitIndexes.toList().sortedBy { (unit, index) -> order[unit] }.toMap()
        ...
        fun getConversions(
            measurementToConvert: String,
            wholeIngredient: String,
            settings: Map<String, String>
        ): List<String> {
            val possibleConversions = mutableListOf<String>()
            //find what type of unit it is
            val splitMeasurement =
                removeUnnecessarySlash(slitTextFromNumbers(getWordsWithNumbers(measurementToConvert)))
            var unitType: CookingUnit? = null
            for (option in unitsLut.entries) {
            ...
            for (value in converted) {
                if (value.key != unitType) {
                    ...
                    possibleConversions.add(
                        "${if (settings["Units.Fractional Numbers"] == "true") vulgarValue.first else value.value}${
                            unitsLut[value.key]?.get(
                                0
                            )
                        }"
                    )
        ...
        private fun getVolumeConversions(
            ml: Float,
            whole: String,
            metricWeight: Boolean,
            settings: Map<String, String>
        ): Map<CookingUnit, Float> {
        ...
        private fun getWeightConversions(
            weight: Float,
            whole: String,
            settings: Map<String, String>
        ): Map<CookingUnit, Float> {
        ...
        private val unitsLut = mapOf(
            ...
            Pair(
                CookingUnit.Tablespoon,
                listOf("tbsp", "tbspn", "tablespoons", "tablespoon", "table spoon")
            ),
         ...
         */
        private fun multiplyByOnlyValues(
            wholeString: String,
            multiplier: Float,
            isVulgar: Boolean,
            roundPercentage: Float
            ...
            //clean the string from brackets and other things in the way
            val cleanWords =
                removeUnnecessarySlash(slitTextFromNumbers(getWordsWithNumbers(wholeString)))
            ...
            for (unit in units) {
                ...
                if (value.matches(numberRegex)) {
                    val newNumber = roundSmallGaps(
                        value.vulgarFraction * multiplier,
                        roundPercentage
                    ).vulgarFraction
                    val replacement: String =
                        if (isVulgar) newNumber.first else newNumber.second.toString()
                    output =
                        output.replaceNumberBeforeValues(value, replacement, unitsLut[unit.key]!!)
                }
            }
            ...
            for (ingredient in ingredients) {
                ...
                if (value.matches(numberRegex)) {
                    ...
                    val replacement: String =
                        if (isVulgar) newNumber.first else newNumber.second.toString()
                    output =
                        output.replaceNumberBeforeValues(value, replacement, listOf(ingredient.key))
                }
            }
        ...
        private fun getWordsWithNumbers(text: String): List<String> {
            return text.lowercase().split("([\\s,;?()-]+)".toRegex())
        } //split at chars that signify a change in word.
        ...
        private fun fixUnits(
            wholeString: String,
            startingValue: String,
            cookingUnitType: CookingUnit,
            settings: Map<String, String>
        ): String {
            ...
            when (cookingUnitType) {
                CookingUnit.Teaspoon -> {
                    ...
                    output = output.replaceNumberBeforeValues(
                        startingValue,
                        (startingValue.vulgarFraction * teaSpoonVolume(settings)).toString(),
                        unitsLut[CookingUnit.Teaspoon]!!
                    )
                    ...
                    output = mlToWeight(
                        (startingValue.vulgarFraction * teaSpoonVolume(settings)).toString(),
                        output,
                        settings["Units.metric Weight"] == "true"
                    )
                    return output
                }
                ...
                CookingUnit.Tablespoon -> {
                    ...
                    output = output.replaceNumberBeforeValues(
                        startingValue,
                        (startingValue.vulgarFraction * tableSpoonVolume(settings)).toString(),
                        unitsLut[CookingUnit.Tablespoon]!!
                    )
                    //replace units
                ...
                CookingUnit.Cup -> {
                    ...
                    output = output.replaceNumberBeforeValues(
                        startingValue,
                        (startingValue.vulgarFraction * cupVolume(settings)).toString(),
                        unitsLut[CookingUnit.Cup]!!
                    )
                    //replace units
                ...
                CookingUnit.Millilitres -> {
                    ...
                    if (settings["Units.Convert To Spoons/Cups"] == "true" && settings["Units.Tea Spoons"] == "true" && startingValue.vulgarFraction < tableSpoonVolume(
                            settings
                        )
                    ) {
                        ...
                        return output
                    } else if (settings["Units.Convert To Spoons/Cups"] == "true" && settings["Units.Table Spoons"] == "true" && startingValue.vulgarFraction < cupVolume(
                            settings
                        ) * 0.25f
                    ) {
                    ...
                    //if not using metric convert it to  fl oz
                        ...
                        output = output.replaceNumberBeforeValues(
                            startingValue,
                            (startingValue.vulgarFraction * 1 / fluidOunceVolume(settings)).toString(),
                            unitsLut[CookingUnit.Millilitres]!!
                        )

                        //replace unit
                        unitsLut[CookingUnit.Millilitres]?.let {
                            output = output.replace(it, "fl oz")
                        }
                        return output
                    }
                ...
                CookingUnit.Litres -> {
                    ...
                    if (settings["Units.metric Volume"] == "false") {
                        ...
                        output = output.replaceNumberBeforeValues(
                            startingValue,
                            (startingValue.vulgarFraction * pintVolume(settings)).toString(),
                            unitsLut[CookingUnit.Litres]!!
                        )
                ...
                CookingUnit.FluidOunce -> {
                    ...
                    if (settings["Units.Convert To Spoons/Cups"] == "true" && settings["Units.Tea Spoons"] == "true" && startingValue.vulgarFraction * fluidOunceVolume(
                            settings
                        ) < tableSpoonVolume(settings)
                    ) { //if smaller than a table spoon use tsp
                        //convert to tsp
                        output = output.replaceNumberBeforeValues(
                            startingValue,
                            (startingValue.vulgarFraction * fluidOunceVolume(settings) * (1 / teaSpoonVolume(
                                settings
                            ))).toString(),
                    ...
                    } else if (settings["Units.Convert To Spoons/Cups"] == "true" && settings["Units.Table Spoons"] == "true" && startingValue.vulgarFraction * fluidOunceVolume(
                            settings
                        ) < cupVolume(settings) * 0.25f
                    ) {//if smaller than a 1/4 cup use table spoon
                        //convert to tbsp
                        output = output.replaceNumberBeforeValues(
                            startingValue,
                            (startingValue.vulgarFraction * fluidOunceVolume(settings) * (1 / tableSpoonVolume(
                                settings
                            ))).toString(),
                            unitsLut[CookingUnit.FluidOunce]!!
                        )
                        ...
                        output = output.replaceNumberBeforeValues(
                            startingValue,
                            (startingValue.vulgarFraction * fluidOunceVolume(settings) * (1 / cupVolume(
                                settings
                            ))).toString(),
                            unitsLut[CookingUnit.FluidOunce]!!
                        )
                        ...
                        output = output.replaceNumberBeforeValues(
                            startingValue,
                            (startingValue.vulgarFraction * fluidOunceVolume(settings)).toString(),
                            unitsLut[CookingUnit.FluidOunce]!!
                        )
                ...
                CookingUnit.Pint -> {
                    if (settings["Units.metric Volume"] == "true") {
                        if (startingValue.vulgarFraction * pintVolume(settings) / 1000 > 1) { //if more than 1 litre convert to litres
                            ...
                            output = output.replaceNumberBeforeValues(
                                startingValue,
                                (startingValue.vulgarFraction * pintVolume(settings) / 1000).toString(),
                                unitsLut[CookingUnit.Pint]!!
                            )
                ...
                CookingUnit.Grams -> {
                    if (settings["Units.metric Weight"] == "false") {
                        //convert to oz
                        output = output.replaceNumberBeforeValues(
                            startingValue, (startingValue.vulgarFraction * 0.03527396f).toString(),
                            unitsLut[CookingUnit.Grams]!!
                        )
                ...
                CookingUnit.KiloGrams -> {
                    if (settings["Units.metric Weight"] == "false") {
                        //convert to pounds
                        output = output.replaceNumberBeforeValues(
                            startingValue, (startingValue.vulgarFraction * 2.204623f).toString(),
                            unitsLut[CookingUnit.KiloGrams]!!
                        )
                ...
                CookingUnit.Ounce -> {
                    if (settings["Units.metric Weight"] == "true") {
                        //convert to grams
                        output = output.replaceNumberBeforeValues(
                            startingValue, (startingValue.vulgarFraction * 28.34952f).toString(),
                            unitsLut[CookingUnit.Ounce]!!
                        )
                ...
                CookingUnit.Pound -> {
                    if (settings["Units.metric Weight"] == "true") {
                        if (startingValue.vulgarFraction * 0.4535924f > 1) {
                            ...
                            output = output.replaceNumberBeforeValues(
                                startingValue,
                                (startingValue.vulgarFraction * 0.4535924f).toString(),
                                unitsLut[CookingUnit.Pound]!!
                            )
                            ...
                            output = output.replaceNumberBeforeValues(
                                startingValue,
                                (startingValue.vulgarFraction * 453.5924f).toString(),
                                unitsLut[CookingUnit.Pound]!!
                            )
        ...
        private fun mlToWeight(valueToChange: String, input: String, metric: Boolean): String {
            ...
            if (metric) {
                ...
                output = input.replaceNumberBeforeValues(
                    valueToChange,
                    (valueToChange.vulgarFraction * conversionValue).toString(),
                    listOf("ml")
                )
                //replace units
                ...
                output = input.replaceNumberBeforeValues(
                    valueToChange,
                    (valueToChange.vulgarFraction * conversionValue * 0.03527396f).toString(),
                    listOf("ml")
                )
                //replace units
            ...
            if (step.container != null) {
                ...
                if (step.container!!.dimensionTwo != null) { //2 dimension size
                    ...
                    text += if (settings["Units.metric Lengths"] == "false") {
                        val convertedValueOne =
                            (step.container!!.dimensionOne!! * 0.3937008f).vulgarFraction //convert to fraction the display the fraction or float depending on settings
                        val convertedValueTwo =
                            (step.container!!.dimensionTwo!! * 0.3937008f).vulgarFraction
                ...
                if (step.container!!.volume != null) {
                    ...
                    text += if (settings["Units.metric Volume"] == "false") {
                        val convertedVal =
                            (step.container!!.volume!! * 1.759754f).vulgarFraction //convert to fraction the display the fraction or float depending on settings
