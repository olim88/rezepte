...
class FileSync {
    ...
    companion object {
        ...
        suspend fun  uploadString(data: Data, file: FileInfo, stringData: String, success: () -> Unit) {
            ...
            if (data.priority == FilePriority.OnlineFirst || data.priority == FilePriority.OnlineOnly || data.priority == FilePriority.Newest || data.priority == FilePriority.None) {
        ...
        suspend fun downloadString(data: Data, file: FileInfo, returnString: (String) -> Unit): Boolean {
                ...
                if (dropbox.second) {
                    ...
                    if (data.priority == FilePriority.Newest) {
                        ...
                        if (onlineDate != null && localData != null) {
                            if (onlineDate.toInstant().toEpochMilli() - localData.second.toInstant()
                                    .toEpochMilli() > 5000
                            ) { //if onlineDate is more than 5 seconds behind exit the program here
                                return true
                            }
                        }
                    }
            ...
            when (data.priority) {
                ...
                FilePriority.Newest -> {
                    ...
                    if (onlineData.second.toInstant().toEpochMilli() - localData.second.toInstant()
                            .toEpochMilli() > 5000
                    ) { //if local is behind by 5seconds return the online file
                        returnString(onlineData.first)
                    }
                }
        ...
        suspend fun downloadThumbnail(
            ...
            val onlineDates = if (data.priority == FilePriority.Newest) {
                val dropbox = getTokenAndOnline(data.dropboxPreference)
                if (dropbox.second) {
                    val downloader = DownloadTask(DropboxClient.getClient(dropbox.first))
            ...
            val onlineData =
                if (data.priority != FilePriority.LocalOnly && data.priority != FilePriority.Newest) {
                    val dropbox = getTokenAndOnline(data.dropboxPreference)
                    if (dropbox.second) {
                        val downloader = DownloadTask(DropboxClient.getClient(dropbox.first))
                ...
                FilePriority.Newest -> {
                    ...
                    for (thumbnailKey in file.fileNames) {
                        if (onlineDates[thumbnailKey] != null) {//if there is a online file
                            ...
                            if (!localThumbnails.contains(thumbnailKey) || localThumbnails[thumbnailKey] == null || onlineDates[thumbnailKey]!!.toInstant()
                                    .toEpochMilli() - localDates[thumbnailKey]?.toInstant()!!
                                    .toEpochMilli() > 5000
                            ) {
            ...
            when (data.priority) {
                ...
                FilePriority.Newest -> {
                    ...
                    if (localFile == null && onlineDate == null) return //no file to sync
                        ...
                        val onlineFile = downloader.getFile(file.dropboxPath, file.fileName)
                        LocalFilesTask.saveFile(onlineFile!!.first, file.localPath, file.fileName)
                    } else if (onlineDate!!.toInstant()
                            .toEpochMilli() - localFile.second.toInstant().toEpochMilli() > 5000
                    ) {//local old upload to local
                        val onlineFile = downloader.getFile(file.dropboxPath, file.fileName)
                        LocalFilesTask.saveFile(onlineFile!!.first, file.localPath, file.fileName)
                ...
                FilePriority.Newest -> {
                    ...
                    for (thumbnailKey in file.fileNames) {
                        if (onlineDates[thumbnailKey] != null) {//if there is a online file
                            //if the online file is newer add to download list
                            if (!localThumbnails.contains(thumbnailKey) || localThumbnails[thumbnailKey] == null || onlineDates[thumbnailKey]!!.toInstant()
                                    .toEpochMilli() - localThumbnails[thumbnailKey]?.second?.toInstant()!!
                                    .toEpochMilli() > 5000
                            ) {
                                neededNames.add(thumbnailKey)
                            }
    ...
    data class FileBatchInfo(
        val dropboxPath: String,
        val localPath: String,
        val fileNames: List<String>
    )

    data class Data(val priority: FilePriority, val dropboxPreference: SharedPreferences)
